  ...
  const handleIncrease = async (item) => {
    try {
      const response = await fetch(
        `https://saranights-server.onrender.com/carts/${item._id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`https://saranights-server.onrender.com/carts/${item._id}`)
          .then((response) => {
            if (response) {
              refetch();
        ...
        <div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start my-12 gap-8">
            ...
            <div className="md:w-1/2 space-y-3">
              ...
              <Link
                to="/process-checkout"
                className="btn btn-md bg-orange text-white px-8 py-1"
              >
